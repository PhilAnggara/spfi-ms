...
@section('content')
    ...
    @if ($rows->isEmpty())
        <p class="center muted" style="margin-top: 8mm;">No delivery receipt records found for the selected filters.</p>
    @else
        <table class="data-table">
            <thead>
                <tr>
                    <th colspan="4" class="center section-head section-sep">Delivery Receipt</th>
                ...
                <tr>
                    ...
                    <th class="section-col col-sep">From</th>
                    <th class="section-col section-sep">To</th>
                    ...
                    <th class="section-col section-start section-sep right">Quantity</th>
            ...
            <tbody>
                @foreach ($rows as $row)
                    <tr>
                        <td>{{ $row['dr_number'] }}</td>
                        <td class="nowrap-date">{{ $fmtTableDate($row['dr_date']) }}</td>
                        <td class="text-wrap-max">{{ $row['from'] }}</td>
                        <td class="text-wrap-max">{{ $row['to'] }}</td>
                        <td>{{ $row['item_code'] }}</td>
