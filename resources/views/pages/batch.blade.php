                ...
                <table class="table table-striped text-center text-nowrap" id="table1">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">Fish Supplier</th>
                            <th class="text-center">Vessel</th>
                            <th class="text-center">Fishing Method</th>
                    ...
                    <tbody>
                        @foreach ($batches as $batch)
                            <tr>
                                <td>
                                    <span class="badge bg-light-secondary" role="button" onclick="copyToClipboard('{{ $batch->code }}')">{{ $batch->code }}</span>
                                </td>
                                <td>{{ $batch->fishSupplier->name ?? '-' }}</td>
                                <td>{{ $batch->vessel->name ?? '-' }}</td>
                                <td>{{ $batch->fishing_method }}</td>
                                <td>{{ $batch->fish_type }}</td>
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        <button type="button" class="btn icon" data-bs-toggle="modal" data-bs-target="#edit-modal-{{ $batch->id }}" data-bstooltip-toggle="tooltip" data-bs-placement="top" title="Edit">
                                            <i class="fa-light fa-edit text-primary"></i>
                                        </button>
                                        <button type="button" class="btn icon" data-bstooltip-toggle="tooltip" data-bs-placement="top" title="Delete" onclick="hapusData({{ $batch->id }}, 'Delete Batch', 'Are you sure want to delete {{ $batch->code }}?')">
                                            <i class="fa-light fa-trash text-secondary"></i>
                                        </button>
                                        <form action="{{ route('batch.destroy', $batch->id) }}" id="hapus-{{ $batch->id }}" method="POST">
                                            @method('delete')
                                            @csrf
                                        </form>
                                    </div>
...
@include('includes.modals.batch-modal')
@endsection
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        @if ($errors->any())
            @if (session('editing_batch_id'))
                const editModal = new bootstrap.Modal(document.getElementById('edit-modal-{{ session("editing_batch_id") }}'));
                editModal.show();
            @else
