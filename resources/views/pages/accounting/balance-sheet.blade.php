...
<div class="page-heading"
    ...
    data-edit-modal-id="{{ $errors->any() && session('editing_balance_sheet_id') ? 'edit-modal' : '' }}">
    ...
    <section class="section">
        <div class="card shadow-sm">
            <div class="card-body">
                <table class="table table-striped text-center text-nowrap" id="balance-sheet-table"
                    data-source="{{ route('accounting.balance-sheet.datatables') }}"
                    data-csrf-token="{{ csrf_token() }}"
                    data-update-route-template="{{ route('accounting.balance-sheet.update', '__ID__') }}"
                    data-destroy-route-template="{{ route('accounting.balance-sheet.destroy', '__ID__') }}"
                    data-open-create-modal="{{ $errors->any() ? '1' : '' }}"
                    data-editing-id="{{ $errors->any() && session('editing_balance_sheet_id') ? '1' : '' }}">
                    <thead>
                        <tr>
...
@push('addon-script')
    <script src="{{ url('assets/scripts/modules/balance-sheet-index.js') }}"></script>
@endpush
