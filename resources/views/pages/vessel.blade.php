                ...
                <table class="table table-striped text-center text-nowrap" id="table1">
                    ...
                    <tbody>
                        @foreach ($vessels as $vessel)
                            <tr>
                                <td>
                                    <span class="badge bg-light-secondary" role="button" onclick="copyToClipboard('{{ $vessel->code }}')">{{ $vessel->code }}</span>
                                </td>
                                <td class="text-start">{{ $vessel->name }}</td>
                                <td>{{ $vessel->creator->name ?? '-' }}</td>
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        <button type="button" class="btn icon" data-bs-toggle="modal" data-bs-target="#edit-modal-{{ $vessel->id }}" data-bstooltip-toggle="tooltip" data-bs-placement="top" title="Edit">
                                            <i class="fa-light fa-edit text-primary"></i>
                                        </button>
                                        <button type="button" class="btn icon" data-bstooltip-toggle="tooltip" data-bs-placement="top" title="Delete" onclick="hapusData({{ $vessel->id }}, 'Delete Vessel', 'Are you sure want to delete {{ $vessel->name }}?')">
                                            <i class="fa-light fa-trash text-secondary"></i>
                                        </button>
                                        <form action="{{ route('vessel.destroy', $vessel->id) }}" id="hapus-{{ $vessel->id }}" method="POST">
                                            @method('delete')
                                            @csrf
                                        </form>
                                    </div>
...
@include('includes.modals.vessel-modal')
@endsection
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        @if ($errors->any())
            @if (session('editing_vessel_id'))
                const editModal = new bootstrap.Modal(document.getElementById('edit-modal-{{ session("editing_vessel_id") }}'));
                editModal.show();
            @else
